...
def dither(cat: Categorical, temp=1.0, eps=0.0, mask=None) -> Categorical:
    ...
    n_valid_outcomes = n_outcomes if mask is None else mask.sum()
    unif_probs = torch.ones(n_outcomes) / n_valid_outcomes
    if mask is not None:
        ...
        for batch_i in (pbar := tqdm(range(n_batches))):
            ...
            for _ in range(batch_size):
                ...
                while not episode.done():
                    ...
                    fmask = state.f_mask()
                    P_valid_action = Categorical(logits=mask_logits(pf_logits, fmask))
                    P_valid_action = dither(P_valid_action, temp=temp, eps=eps, mask=fmask)
